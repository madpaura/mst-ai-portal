...
async def list_capabilities():
    async def _fetch():
        db = await get_db()
        rows = await db.fetch("SELECT * FROM capabilities WHERE is_active = true ORDER BY sort_order")
        return [
            CapabilityResponse(id=str(r["id"]), icon=r["icon"], title=r["title"],
                               description=r["description"], sort_order=r["sort_order"]).model_dump(mode="json")
            for r in rows
        ]
    return await cache.get_or_set(cache.NS_SOLUTIONS, "list", "capabilities", None, settings.REDIS_DEFAULT_TTL, _fetch)
...
async def list_announcements():
    async def _fetch():
        db = await get_db()
        rows = await db.fetch("SELECT * FROM announcements WHERE is_active = true ORDER BY created_at DESC LIMIT 10")
        return [
            AnnouncementResponse(id=str(r["id"]), title=r["title"], content=r.get("content"),
                                 badge=r.get("badge"), created_at=r["created_at"]).model_dump(mode="json")
            for r in rows
        ]
    return await cache.get_or_set(cache.NS_SOLUTIONS, "list", "announcements", None, settings.REDIS_DEFAULT_TTL, _fetch)
...
async def list_solution_cards():
    async def _fetch():
        db = await get_db()
        rows = await db.fetch("SELECT * FROM solution_cards WHERE is_active = true ORDER BY sort_order LIMIT 8")
        return [
    ...
    return await cache.get_or_set(cache.NS_SOLUTIONS, "list", "cards", None, settings.REDIS_DEFAULT_TTL, _fetch)
...
async def list_news_feed():
    async def _fetch():
        db = await get_db()
        rows = await db.fetch("SELECT * FROM news_feed WHERE is_active = true ORDER BY published_at DESC LIMIT 20")
        return [
            NewsFeedResponse(
    ...
    return await cache.get_or_set(cache.NS_SOLUTIONS, "list", "news", None, settings.REDIS_DEFAULT_TTL, _fetch)
